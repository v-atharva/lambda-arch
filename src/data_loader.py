"""
Unified data access layer for the banking system.

Provides the DataLoader class which can load customers, cards, card types,
and transactions from either the MySQL database (serving layer) or
fallback CSV files in the dataset/ directory.
"""
...
class DataLoader:
    """Loads banking data from MySQL or CSV depending on configuration."""

    def __init__(self, data_dir: Path = Path("../data"), use_db: bool = True):
        """Set data source: use_db=True for MySQL, False for CSV files."""
        self.data_dir = data_dir
        self.use_db = use_db
        self.db_manager = DBManager() if use_db else None

    def load_customers(self) -> Dict[int, Customer]:
        """Return a dict of customer_id -> Customer from DB or CSV."""
        customers = {}
    ...
    def load_card_types(self) -> Dict[int, CardType]:
        """Return a dict of card_type_id -> CardType from DB or CSV."""
        card_types = {}
    ...
    def load_cards(self) -> Dict[int, Card]:
        """Return a dict of card_id -> Card from DB or CSV."""
        cards = {}
